...
int main(){
	float d, b, Vu, fck, fy, n, n_b, dia, alpha;
	...
	cout<<"fy = "<<flush;
	cin>>fy;
	cout<<"Number of normal bars = "<<flush;
	cin>>n;
	cout<<"Number of bentup bars = "<<flush;
	cin>>n_b;
	...
	cout<<"Tcmax = 0.62*root(fck)"<<" = 0.62*root"<<fck<<" = "<<Tcmax<<endl;
	if(Tv<Tcmax){
	...
	if(Tv<=Tc){
		...
		cin>>dia_s;
		Vus = (Tv - Tc)*b*d;
		cout<<"Shear resisted by shear r/f, Vus = (Tv - Tc)*b*d"<<" = "<<Vus/1000<<"kN"<<endl<<endl;
		cout<<"so, we divide Vus 50%-50% in bent-up bars and stirrups"<<endl;
		
		cout<<"Shear resisted by bent up bars"<<endl;
		alpha = alpha*3.14159/180;
		float Vus_bentup = 0.87*fy*((n_b*M_PI*pow(dia, 2))/4)*sin(alpha);
		cout<<"Vus = 0.87*fy*Asv*sin(alpha)"<<endl;
		cout<<"Vus = 0.87*"<<fy<<"*"<<n_b<<"*(pi/4)*"<<dia<<"^2*"<<sin(alpha)<<endl;
		cout<<"Vus = "<<Vus_bentup<<"N = "<<Vus_bentup/1000<<"KN"<<endl;
		if((float)(Vus/2.0)<=Vus_bentup){
			cout<<"Ok"<<endl;
		}cout<<endl;
		
		cout<<"Provide 2 leggerd --mm -----"<<endl;
		float Vus_stirrup = Vus/2;
		cout<<"Sv = (0.87*fy*Asv*d)/Vus"<<endl;
		cout<<"Sv = "<<"0.87*"<<fy_s<<"*"<<n_s<<"(pi/4)*"<<dia_s<<"^2*"<<d<<"/"<<Vus_stirrup<<endl;
		cout<<"Sv = "<<(0.87*fy*((n_s*M_PI*pow(dia_s, 2))/4)*d)/Vus_stirrup<<endl;
		...
		float b = 0.75*d;
		float c = 300;
		float e = (0.87*fy*((n_s*M_PI*pow(dia_s, 2))/4)*d)/Vus_stirrup;
		cout<<"Sv = min("<<a<<", "<<b<<", "<<d<<", 300)"<<endl;
		cout<<"Sv = "<<min(a, min(b, min(c, e)))<<endl;
		cout<<"Provide --- ---------- at "<<Sv<<endl;
	}
